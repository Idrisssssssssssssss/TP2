...
function check_sold()
{
  ...
  if(achats <= budget)
  {
    alert("Vous pouvez tout acheter avec le budget dont vous disposez, cela vous permettra " + (budget - achats) + " €")
  }
  if(achats > budget)
  {
    alert("Vous ne pouvez pas tout acheter avec le budget dont vous disposez, dont vous avez besoin  " + math.abs(budget - achats) + " € more")
  }
}
...
function get_ttc_v2()
{
  ...
  alert("Votre prix ttc est " + Math.ceil(prix * ( 1 - (tva/100))));
}
...
function get_random_v2()
{
  ...
  for(i = 1; i <= random_nums; i++)
  {
    ...
    if (random == 10)
    {
      console.log(`${random} c'était ce que nous recherchions !`);
    }
